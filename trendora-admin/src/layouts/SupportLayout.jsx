...
import { LayoutDashboard, Inbox, Headphones, MessageCircle, Bell, LogOut } from 'lucide-react';

const links = [
  { to: '/support/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { to: '/support/inbox', icon: Inbox, label: 'Inbox' },
  { to: '/support/tickets', icon: Headphones, label: 'All Tickets' },
  { to: '/support/messages', icon: MessageCircle, label: 'Messages' },
];

function getNotifRedirect(type) {
  if (type === 'ticket') return '/support/inbox';
...
export default function SupportLayout({ children }) {
  ...
  const initials = user?.fullName ? user.fullName.split(' ').map((n) => n[0]).join('').toUpperCase() : user?.username?.[0]?.toUpperCase() || 'S';

  return (
    <div className="app-layout">
      <aside style={{ width: 260, background: '#ffffff', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 100, borderRight: '1px solid #e5e7eb' }}>
        <div style={{ padding: '24px 20px', borderBottom: '1px solid #f3f4f6' }}>
          <h1 style={{ fontSize: 20, fontWeight: 800, color: '#059669', letterSpacing: '-0.5px' }}>Trend<span style={{ color: '#d4af37' }}>ora</span></h1>
          <p style={{ fontSize: 11, color: '#6b7280', marginTop: 4, textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600 }}>Support Portal</p>
        </div>
        <nav style={{ flex: 1, padding: '12px 8px', overflowY: 'auto' }}>
          {links.map((link) => (
            <NavLink key={link.to} to={link.to}
              style={({ isActive }) => ({ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 8, color: isActive ? '#059669' : '#374151', textDecoration: 'none', fontSize: 14, fontWeight: 500, background: isActive ? 'rgba(5,150,105,0.08)' : 'transparent', transition: 'all 0.2s ease' })}>
        ...
        <div style={{ padding: '16px 12px', borderTop: '1px solid #f3f4f6' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px', borderRadius: 8, cursor: 'pointer' }} onClick={handleLogout}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #059669, #10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 13, fontWeight: 700 }}>{initials}</div>
            <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{user?.fullName || user?.username || 'Support'}</div><div style={{ fontSize: 11, color: '#d4af37', fontWeight: 500 }}>Support Agent</div></div>
      ...
      <main style={{ flex: 1, marginLeft: 260, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <div style={{ height: 56, background: '#fff', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 24px', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ textAlign: 'right' }}><div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{user?.fullName || user?.username}</div><div style={{ fontSize: 11, color: '#059669', fontWeight: 500 }}>Support Agent</div></div>
          </div>
        </div>
